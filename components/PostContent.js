    ...
    <div>
      <div className="flex w-full">
        ...
        <div className="pl-2 grow">
          <div>
            ...
            {big && (<br />)}
            <Link href={'/'+author?.username}>
              <span className="text-nexusLightGray cursor-pointer">@{author?.username}</span>
            </Link>
            {createdAt && !big && (
              <span className="pl-1 text-nexusLightGray">
              <ReactTimeAgo date={createdAt} timeStyle={'nexus'} />
            </span>
            )}
          </div>
          {!big && (
      ...
      {big && (
        <div className="mt-2">
          ...
          {createdAt && (
            <div className="text-nexusLightGray text-sm">
              {(new Date(createdAt))
                .toISOString()
                .replace('T', ' ')
                .slice(0,16)
