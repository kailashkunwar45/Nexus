            ...
            <div className="p-2">
              {!isMyProfile && (
                <button onClick={toggleFollow}
                        className={(isFollowing ? 'bg-nexusWhite text-black' : 'bg-nexusAccent text-white')+" py-2 px-5 rounded-full"}>
                ...
                <div>
                  {!editMode && (
                    <button
                      ...
                      className="bg-nexusAccent text-white py-2 px-5 rounded-full">Edit profile</button>
                  )}
                  {editMode && (
                    <div>
                      <button
                        onClick={() => cancel()}
                        className="bg-nexusWhite text-black py-2 px-5 rounded-full mr-2">Cancel</button>
                      <button
                        onClick={() => updateProfile()}
                        className="bg-nexusAccent text-white py-2 px-5 rounded-full">Save profile</button>
                    </div>
                  )}
                </div>
              )}
            ...
            {editMode && (
              <div>
                <input type="text" value={profileInfo.name}
                       ...
                       className="bg-nexusBorder p-2 mb-2 rounded-full"/>
              </div>
            )}
            {!editMode && (
            ...
            {editMode && (
              <div>
                <textarea value={profileInfo.bio}
                          ...
                          className="bg-nexusBorder p-2 mb-2 rounded-2xl w-full block"  />
              </div>
            )}
          </div>
        </div>
